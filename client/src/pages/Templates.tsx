...
export const Templates: React.FC<TemplatesProps> = ({
  ...
  const handleCreateNew = () => {
    setEditingTemplate({
      ...
      description: '',
      subjectTemplate: '',
      htmlContent: `<div style="font-family: sans-serif; padding: 24px; background: #0f172a; color: #f8fafc; border-radius: 12px;">
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Standardized transactional HTML templates with dynamic mustache <code className="text-cyan-400 font-mono font-semibold">{'{{placeholder}}'}</code> variables.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {templates.map((tpl) => (
          <div key={tpl.id} className="glass-card p-5 flex flex-col justify-between border-slate-800">
            <div>
              <div className="flex items-start justify-between gap-2">
                <div>
                  <h3 className="text-sm font-bold text-white flex items-center gap-2">
                    {tpl.name}
                    {tpl.isBuiltIn && (
                      <span title="Built-in Standard Template" className="text-cyan-400">
                        <Lock className="w-3 h-3" />
                      </span>
                    )}
                  </h3>
                  <code className="text-[11px] font-mono text-cyan-400 bg-slate-950 px-1.5 py-0.5 rounded mt-1 inline-block">
                    {tpl.slug}
                  </code>
                </div>
                <span
              ...
              {/* Subject */}
              <div className="mt-3 p-2.5 bg-slate-950/80 rounded-lg border border-slate-800/80 text-xs">
                <span className="text-[10px] uppercase font-bold text-slate-500 block mb-0.5">Subject Format</span>
                <span className="text-slate-300 font-mono text-[11px] truncate block">{tpl.subjectTemplate}</span>
              </div>

              {/* Variables */}
              <div className="mt-3">
                <span className="text-[10px] uppercase font-bold text-slate-500 block mb-1.5">
                  Available Variables ({tpl.variables.length})
                </span>
                <div className="flex flex-wrap gap-1 max-h-20 overflow-y-auto">
                  {tpl.variables.map((v) => (
                    <span
                      key={v}
                      className="text-[10px] font-mono px-2 py-0.5 bg-slate-900 text-slate-300 border border-slate-800 rounded-md"
                    >
                      {`{{${v}}}`}
                    </span>
                  ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fadeIn">
          <div className="w-full max-w-4xl max-h-[90vh] bg-slate-900 border border-slate-800 rounded-2xl flex flex-col shadow-2xl overflow-hidden">
            ...
            <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-6 space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">HTML Template Content</label>
                <textarea
